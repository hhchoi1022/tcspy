...
if __name__ == '__main__':
    ...
    # Dark for exoplanet
    A.schedule_app(A.run_dark, A.obsnight.sunrise_flat + 65 * u.minute, count = 9, exptime = 8, binning = 1, gain = 0, alert_slack = alert_slack)

    # Bias
    A.schedule_app(A.run_bias, A.obsnight.sunrise_flat + 85 * u.minute, count = 9, binning = 1, gain = 2750, alert_slack = alert_slack)
    # Bias
    A.schedule_app(A.run_bias, A.obsnight.sunrise_flat + 95 * u.minute, count = 9, binning = 1, gain = 0, alert_slack = alert_slack)


    # Shutdown
    A.schedule_app(A.run_shutdown, A.obsnight.sunrise_flat + 105 * u.minute, fanoff = True, slew = True, warm = True, alert_slack = alert_slack)
