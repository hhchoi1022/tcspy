...
if __name__ == '__main__':
    ...
    if A.obsnight.sunrise_startup.datetime.day % 3 == 0:
        ...
        A.schedule_app(A.run_flat, A.obsnight.sunrise_flat, count = 9, binning = 2, gain = 0, alert_slack = alert_slack)
    ...
    # Dark for exoplanet
    A.schedule_app(A.run_dark, A.obsnight.sunrise_flat + 65 * u.minute, count = 9, exptime = 8, binning = 2, gain = 0, alert_slack = alert_slack)

    # Bias
    A.schedule_app(A.run_bias, A.obsnight.sunrise_flat + 85 * u.minute, count = 9, binning = 1, gain = 2750, alert_slack = alert_slack)
    # Bias
    A.schedule_app(A.run_bias, A.obsnight.sunrise_flat + 95 * u.minute, count = 9, binning = 2, gain = 0, alert_slack = alert_slack)


    # Shutdown
    A.schedule_app(A.run_shutdown, A.obsnight.sunrise_flat + 1055 * u.minute, fanoff = True, slew = True, warm = True, alert_slack = alert_slack)
